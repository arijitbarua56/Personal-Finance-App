...
function transferEffectiveForDate(d){return !!state.settings?.transferReceived}
...
function renderLumpSumAllocation(){if(!$("lumpSumAllocation"))return;const rec=recommendedLumpAllocation(),wrap=$("lumpSumAllocation");wrap.innerHTML="";
 const badge=$("transferStatusBadge");if(badge){if(state.settings?.transferReceived){badge.textContent="Received";badge.className="badge ok"}else{badge.textContent="Not received · expected "+parseDateOnly(state.settings?.transferDate||"2026-10-15").toLocaleDateString(undefined,{month:"short",day:"numeric"});badge.className="badge warn"}}
